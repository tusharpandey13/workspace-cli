...
import { GitHubApiClient } from '../services/githubApiClient.js';
...
export async function validateGitHubIdsExistence(
  ...
  for (const issueId of issueIds) {
    try {
      ...
      // Fetch the issue using the API client
      const issue = await apiClient.getIssue(githubOrg, repoName, issueId);

      // Verify the returned issue number matches
      if (issue.number !== issueId) {
        throw new Error(`Issue ID mismatch: expected ${issueId}, got ${issue.number}`);
      }
      ...
      if (errorMessage.includes('Network error') || errorMessage.includes('timeout')) {
        throw new Error(
          `❌ GitHub API request failed for issue #${issueId}. Please check your network connection and try again.`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          `❌ Access denied to ${githubOrg}/${repoName}. Please ensure GITHUB_TOKEN is set with appropriate permissions.`,
        );
      }

      if (errorMessage.includes('GITHUB_TOKEN')) {
        throw new Error(
          `❌ GITHUB_TOKEN is not set. Please set it as an environment variable with appropriate permissions.`,
        );
      }
