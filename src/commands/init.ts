...
async function initializeWorkspace(options: InitializeWorkspaceOptions): Promise<void> {
  ...
  const githubData = await fetchGitHubData(issueIds, project.github_org, project.sdk_repo, isDryRun);
  ...
  const summary = issueIds.length > 0 
...
async function handleExistingWorkspace(workspaceDir: string, isDryRun: boolean): Promise<void> {
  if (!isDryRun && fs.existsSync(workspaceDir) && (await fs.readdir(workspaceDir)).length > 0) {
    ...
    const answer = (await rl.question(`Workspace ${workspaceDir} already exists. Clean and overwrite? [y/N] `)).trim().toLowerCase();
    rl.close();
...
async function collectAdditionalContext(): Promise<string[]> {
  ...
  console.log('Enter any additional context that might be helpful for analysis (press Enter on empty line to finish):');
...
/**
 * Fetch GitHub data for a single issue/PR
 */
async function fetchSingleGitHubItem(githubId: number, githubOrg: string, repoName: string, isDryRun: boolean): Promise<any> {
  logger.verbose(`📊 Fetching data for GitHub ID #${githubId}...`);
  
  // Try issue first, then PR
  const endpoints = [
    `repos/${githubOrg}/${repoName}/issues/${githubId}`,
    `repos/${githubOrg}/${repoName}/pulls/${githubId}`
  ];
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (!isDryRun) {
        ...
        return createMockGitHubData(githubId, endpoint.includes('pulls') ? 'pull_request' : 'issue');
      }
    } catch {
      // Try next endpoint
    }
  ...
  throw new Error(`Failed to fetch data for #${githubId}`);
}

/**
 * Fetch GitHub data for issues/PRs
 */
async function fetchGitHubData(issueIds: number[], githubOrg: string, repoName: string, isDryRun: boolean): Promise<GitHubIssueData[]> {
  const githubData: GitHubIssueData[] = [];
  
  for (const githubId of issueIds) {
    try {
      const data = await fetchSingleGitHubItem(githubId, githubOrg, repoName, isDryRun);
      const extracted = extractRelevantContent(data);
      
      logger.verbose(`📝 Fetching comments for #${githubId}...`);
      extracted.comments = await fetchComments(data.comments_url, githubId, isDryRun);
      githubData.push(extracted);
    } catch (err) {
      logger.warn(`Error processing GitHub ID "${githubId}": ${(err as Error).message}`);
    }
  }
 ...
 */
function createMockGitHubData(githubId: number, type: string): any {
  return {
    number: githubId,
    title: type === 'pull_request' ? 'Add new session refresh mechanism' : 'Fix authentication middleware token validation error',
    body: 'Dry run data - this would contain the actual issue/PR body with detailed description',
    state: 'open',
    html_url: `https://github.com/auth0/repo/${type === 'pull_request' ? 'pull' : 'issues'}/${githubId}`,
    created_at: '2025-01-01T00:00:00Z',
    updated_at: '2025-01-01T00:00:00Z',
    comments_url: `https://api.github.com/repos/auth0/repo/issues/${githubId}/comments`,
 ...
 */
async function setupEnvironmentAndDependencies(project: ProjectConfig, projectKey: string, paths: WorkspacePaths, isDryRun: boolean): Promise<void> {
...
function createPlaceholderValues({
  ...
  const githubDataFormatted = githubData.length > 0 
    ...
    : 'No GitHub issues or PRs provided.';
  
  const additionalContextFormatted = additionalContext.length > 0
...
function generateMockSampleKeyFiles(project: ProjectConfig): string {
  ...
  return files
    .map((rel) => `- ${sampleAppPath}/${rel} (mock - analysis mode)`)
    .join('\n');
}
...
`;
    await fileOps.writeFile(bugReportPath, fileContent, primaryIssueId ? `bug report for #${primaryIssueId}` : `workspace info for ${branchName}`, isDryRun);
  }
}
 ...
 */
async function handlePRInitialization(projectKey: string, prIdStr: string, options: { verbose?: boolean; dryRun?: boolean; analyse?: boolean }): Promise<void> {
  ...
  if (isAnalyseMode) {
    throw new Error('Analysis mode (--analyse) is not yet supported with PR initialization. Use regular init with --analyse instead.');
  }
...
export function initCommand(program: Command): void {
  program
    ...
    .argument('<project>', 'Project key (e.g., next, node, react) - use "workspace projects" to see available projects')
    .argument('[args...]', 'GitHub issue IDs followed by branch name, or just branch name (optional when using --pr)')
    .option('-v, --verbose', 'Enable verbose logging output')
    .option('--dry-run', 'Show what would be done without executing (implies verbose)')
    .option('--analyse', 'Analysis mode: create workspace and populate prompt files without setting up git worktrees')
    .addHelpText('after', `
Examples:
...
Related commands:
  ...
  workspace info        Show workspace details`)
    .action(async (projectKey: string, args: string[], options: { verbose?: boolean; dryRun?: boolean; analyse?: boolean }) => {
      try {
        ...
        if (!args || args.length === 0) {
          throw new Error('Branch name is required when not using --pr option. Usage: init <project> <branch-name> OR init <project> --pr <pr-id>');
        }
        ...
        const initMessage = issueIds.length > 0
