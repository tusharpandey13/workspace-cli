...
`,
    ...
    const warmResult: BenchmarkResult = {
      ...
      value: warmTime,
      unit: 'ms',
      target: coldTime * 0.5, // Should be at least 50% faster
    };
  ...
  async benchmarkWorkspaceInit(): Promise<BenchmarkResult> {
    ...
    await this.runCLI(['init', '--no-config', '--dry-run', 'next', 'benchmark-branch']);
  ...
  validatePerformance(): { passed: boolean; issues: string[] } {
    ...
    for (const result of this.results) {
      ...
      if (result.baseline && result.value > result.baseline) {
        issues.push(
          `${result.metric}: ${result.value}${result.unit} regression from baseline ${result.baseline}${result.unit}`,
        );
      }
    }
...
describe('Performance Benchmarking Suite', () => {
  ...
  it('should meet startup time performance targets', async () => {
    ...
    console.log(`Startup time: ${result.value.toFixed(2)}ms (target: <${result.target}ms)`);
  }, 30000);
  ...
  it('should demonstrate config caching performance improvement', async () => {
    ...
    expect(warmResult.value).toBeLessThan(warmResult.target!);

    const improvement = (((coldResult.value - warmResult.value) / coldResult.value) * 100).toFixed(
      1,
    );
    console.log(`Config caching improvement: ${improvement}%`);
  }, 15000);
