...
`,
  ...
  async testWorkspaceInit(): Promise<number> {
    ...
    try {
      await this.runCLI(['init', '--no-config', '--dry-run', 'next', 'perf-test-branch']);
...
describe('Performance Integration Tests', () => {
  ...
  it('should maintain fast startup time with all optimizations', async () => {
    ...
    // Should be under 100ms (much better than original 200ms target)
  ...
  it('should maintain low memory usage', async () => {
    ...
    // Should be under 5MB (current target)
  ...
  it('should show config caching performance improvement', async () => {
    ...
    // Cached load should be significantly faster than first load
    expect(cachedLoad).toBeLessThan(firstLoad * 0.5); // At least 50% improvement
  ...
  it('should maintain performance with progress indicators enabled', async () => {
    ...
    try {
      await tester.runCLI(['init', '--no-config', '--dry-run', 'next', 'progress-perf-test']);
